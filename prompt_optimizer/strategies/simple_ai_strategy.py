...
class SimpleAIStrategy(OptimizationStrategy):
    ...
    def is_ready_for_optimization(self, 
        ...
        try:
            ...
            # Calculate positive rate based on score
    ...
    def _prepare_examples(self, 
        ...
        try:
            ...
            # Get a mix of high and low scored examples
            positive = [item for item in sorted_data if (item.get("score") or 0) >= 0.5][:limit//2]
            negative = [item for item in sorted_data if (item.get("score") or 0) < 0.5][:limit//2]
...
"""
            ...
            for i, example in enumerate(examples, 1):
                feedback_type = "👍 HIGH" if (example.get("score") or 0) >= 0.5 else "👎 LOW"
                ai_prompt += f"""
